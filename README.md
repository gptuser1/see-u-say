...
Text-to-Image 测试站，用于测试提示词对 FLUX.2 klein 4B 模型的生成效果。部署在 Cloudflare Pages。
...
```bash
npx wrangler pages dev . --binding CF_ACCOUNT_ID=xxx --binding CF_API_TOKEN=xxx
```

## 说明

- 图片模型固定 4 步推理，不可调
- 参考图字段名使用 `input_image_0` ~ `input_image_3`（与 Cloudflare flux-2 REST API 一致）
- 思考模式同时冗余传入 `thinking` 与 `enable_thinking` 两套字段，兼容不同提供商
